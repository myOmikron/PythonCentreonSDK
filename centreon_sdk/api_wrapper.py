...
class ApiWrapper:
    ...
    def host_add(self, host_name, host_alias, host_address, host_templates, instance, host_groups):
        ...
        data_dict = {"action": "add",
                     "object": "host",
                     "values": ";".join([host_name, host_alias, host_address, "|".join(host_templates)
                     if isinstance(host_templates, list) else host_templates, instance,
                                         "|".join(host_groups) if isinstance(host_groups, list) else host_groups])}
        response = self.network.make_request(HTTPVerb.POST, params=self.config.vars["params"], data=data_dict)
    ...
    def host_del_contact(self, host_name, contacts):
    ...
    def host_set_host_group(self, host_name, host_group_names):
        ...
        data_dict = {"action": "sethostgroup",
                     "object": "host",
                     "values": ";".join([host_name, "|".join(host_group_names) if isinstance(host_group_names, list)
        ...
        for downtime in response:
            ...
            if "service_groups" in downtime:
                if isinstance(downtime["service_groups"], list):
                    ...
                    downtime["service_groups"] = [downtime["service_groups"]] if len(
                        downtime["service_groups"]) > 0 else []
    ...
    def downtime_list_periods(self, downtime_name):
        ...
        for period in response:
            ...
            period["day_of_week"] = [int(x) for x in period["day_of_week"].split(',')] if len(
                period["day_of_week"]) > 0 else []
            period["day_of_month"] = [int(x) for x in period["day_of_month"].split(',')] if len(
                period["day_of_month"]) > 0 else []
    ...
    def real_time_downtime_add_service(self, service_names, start_time, end_time, fixed, duration, description):
        ...
        data_dict = {"action": "add",
                     "object": "rtdowntime",
                     "values": ";".join(
                         ["SVC", "|".join(service_names), start_time, end_time, str(int(fixed)), duration,
                          description])}
        response = self.network.make_request(HTTPVerb.POST, params=self.config.vars["params"], data=data_dict)
        return method_utils.check_if_empty_list(response)
    ...
    def service_template_show(self):
        ...
        for service_template in response:
            ...
            service_template["active_checks_enabled"] = ThreeWayOption.NO if service_template[
                                                                                 "active_checks_enabled"] == "0" \
                else ThreeWayOption.YES if service_template["passive_checks_enabled"] == "1" else ThreeWayOption.DEFAULT
            service_template["passive_checks_enabled"] = ThreeWayOption.NO if service_template[
